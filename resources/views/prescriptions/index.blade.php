...
<div class="container">
    <div class="page-header">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h2><i class="fas fa-prescription me-2"></i>{{ __('messages.prescriptions') }}</h2>
            </div>
            <div>
                @can('create', App\Models\Prescription::class)
                <a href="{{ route('admin.prescriptions.create') }}" class="btn btn-add">
                    <i class="fas fa-plus me-2"></i>{{ __('messages.add_prescription') }}
                </a>
                @endcan
            </div>
        </div>
                    ...
                    <tbody>
                        @forelse($prescriptions as $prescription)
                            <tr>
                                ...
                                <td>{{ $prescription->items->count() }} {{ __('messages.items') }}</td>
                                <td>
                                    <div class="action-buttons">
                                        @can('view', $prescription)
                                        <a href="{{ route($prescriptionsRoute . '.show', $prescription) }}" class="btn btn-info" title="{{ __('messages.view') }}">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        @endcan
                                        @can('update', $prescription)
                                        <a href="{{ route('admin.prescriptions.edit', $prescription) }}" class="btn btn-primary" title="{{ __('messages.edit') }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="empty-state">
                                    ...
                                    <h5 class="text-muted mb-2">{{ __('messages.no_prescriptions_found') }}</h5>
                                    @can('create', App\Models\Prescription::class)
                                    <a href="{{ route('admin.prescriptions.create') }}" class="btn btn-add mt-3">
                                        <i class="fas fa-plus me-2"></i>{{ __('messages.add_prescription') }}
                                    </a>
                                    @endcan
                                </td>
