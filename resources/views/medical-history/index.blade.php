...
@section('title', __('messages.medical_history'))
...
<style>
    ...
    .action-buttons .btn-info:hover {
        background: linear-gradient(135deg, #3ad86b 0%, #2ee9c7 100%);
        color: white;
    }
...
<div class="container">
    <div class="page-header">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h2><i class="fas fa-history me-2"></i>{{ __('messages.medical_history') }}</h2>
            </div>
            <div>
                @can('create', App\Models\MedicalHistory::class)
                <a href="{{ route('admin.medical-history.create') }}" class="btn btn-add">
                    <i class="fas fa-plus me-2"></i>{{ __('messages.add_medical_history') }}
                </a>
                @endcan
            </div>
        </div>
                        ...
                        @forelse($medicalHistories as $history)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @endphp
                                    <span class="badge bg-info">{{ $types[$history->record_type] ?? $history->record_type }}</span>
                                </td>
                                <td>{{ $history->title }}</td>
                                <td>{{ $history->date ? $history->date->format('Y-m-d') : '-' }}</td>
                                <td>{{ $history->doctor ? $history->doctor->name : '-' }}</td>
                                <td>
                                    <div class="action-buttons">
                                        @can('view', $history)
                                        <a href="{{ route($medicalHistoryRoute . '.show', $history) }}" class="btn btn-info" title="{{ __('messages.view') }}">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        @endcan
                                        @can('update', $history)
                                        <a href="{{ route('admin.medical-history.edit', $history) }}" class="btn btn-primary" title="{{ __('messages.edit') }}">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        @endcan
                                        @can('delete', $history)
                                        <form action="{{ route('admin.medical-history.destroy', $history) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="empty-state">
                                    ...
                                    <h5 class="text-muted mb-2">{{ __('messages.no_medical_history_found') }}</h5>
                                    @can('create', App\Models\MedicalHistory::class)
                                    <a href="{{ route('admin.medical-history.create') }}" class="btn btn-add mt-3">
                                        <i class="fas fa-plus me-2"></i>{{ __('messages.add_medical_history') }}
                                    </a>
                                    @endcan
                                </td>
