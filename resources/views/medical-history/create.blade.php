...
@section('title', __('messages.add_medical_history'))
...
<div class="container">
    <div class="page-header">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h2><i class="fas fa-plus me-2"></i>{{ __('messages.add_medical_history') }}</h2>
            </div>
            <div>
    ...
    <div class="card form-card">
        <div class="card-body">
            <form action="{{ route('admin.medical-history.store') }}" method="POST">
                @csrf
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('record_type') is-invalid @enderror" id="record_type" name="record_type" required>
                                ...
                                <option value="allergy" {{ old('record_type') == 'allergy' ? 'selected' : '' }}>{{ __('messages.allergy') }}</option>
                                <option value="surgery" {{ old('record_type') == 'surgery' ? 'selected' : '' }}>{{ __('messages.surgery') }}</option>
                                <option value="chronic_disease" {{ old('record_type') == 'chronic_disease' ? 'selected' : '' }}>{{ __('messages.chronic_disease') }}</option>
                                <option value="medication" {{ old('record_type') == 'medication' ? 'selected' : '' }}>{{ __('messages.medication_history') }}</option>
                                <option value="vaccination" {{ old('record_type') == 'vaccination' ? 'selected' : '' }}>{{ __('messages.vaccination') }}</option>
                                <option value="other" {{ old('record_type') == 'other' ? 'selected' : '' }}>{{ __('messages.other') }}</option>
                            </select>
                            @error('record_type')
                                <div class="invalid-feedback d-block">
                                    <i class="fas fa-exclamation-circle me-1"></i>{{ $message }}
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('attachments') is-invalid @enderror" id="attachments" name="attachments" value="{{ old('attachments') }}" placeholder="رابط أو مسار الملف">
                            @error('attachments')
                                <div class="invalid-feedback d-block">
                                    <i class="fas fa-exclamation-circle me-1"></i>{{ $message }}
                                </div>
